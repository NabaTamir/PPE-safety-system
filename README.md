# Mining Safety — PPE Compliance Detection (YOLO11 + NVIDIA LocateAnything-3B)
...
in a single step, which makes it roughly 10× faster than other generative vision-language models —
...
1. **Removed the noisy `undefined` class.** The raw export contained a 7th `undefined` class that
   scored mAP50 ≈ 0.24 and dragged the whole model down; its labels were stripped and the
...
| Input / classes | 768×768, 6 classes |
...
| Hardware | NVIDIA A100-40GB (Google Colab), ~2.8 h, checkpointed to Drive |
...
- **Hardware.** Needs an Ampere/Lovelace GPU (L4 / A100); ~1–3 s per frame, so video is processed
  by **frame-sampling**, not in real time. License is NVIDIA *non-commercial / research only*.
...
epoch 60–90** — most learning happens in the first half, and extra epochs give little gain.
...
**Video inference** runs frame-by-frame (~25 ms/frame on an Apple-Silicon GPU):
...
*Fig. 6. LocateAnything-3B annotating a video by frame-sampling (~1–3 s/frame).*
...
rank by (compare YOLO11's `0.87`, `0.75`, …). Its boxes are **looser** than the trained model's,
and the **negation prompts are unreliable** — `person without helmet` fires even on workers who are
clearly wearing a helmet (Fig. 5). It also
occasionally emits whole-frame noise boxes (filtered out in post-processing), needs a large GPU,
and is far too slow for real-time video. Because the labels are unstable and unscored, it does
**not** plug cleanly into the geometric compliance layer the way the trained detector does.
...
checkpointing) gave smooth, stable training and a respectable **mAP50 ≈ 0.77** on completely
unseen data.
...
(`no-vest`, `no-boot`) is only ~0.55 — the model misses almost half of them — a direct result of
having ~90 examples of these versus ~9,000 vests. **Second**, the dataset itself has **label
...
examples (`no-helmet`, `no-vest`, `no-boot`) and clean the existing labels — this is what would
move the score from ~0.77 toward ~0.85+. The open-vocabulary model suggests a concrete way to do
this cheaply: use **LocateAnything to auto-propose boxes** for rare or novel items, then have a
...
├── test/  train/                     # Image data (gitignored)
├── models/                           # Trained weights (gitignored)
└── result/                           # Inference outputs (gitignored)
```

**Run locally** (after exporting `best.pt` from the notebook):

```bash
pip install ultralytics
python predict.py --weights models/best_v2.pt --source test/test_image --device mps
python compliance_rules.py --weights models/best_v2.pt --source test/test_video/video_1.mp4 --device mps
```

> The Roboflow API key is read from a Colab secret / environment variable (`ROBOFLOW_API_KEY`) —
> never hard-coded. The image dataset and trained weights are gitignored (large / re-downloadable).
> LocateAnything-3B runs in Colab on an L4 / A100 GPU (see the notebook).
